 ...
 */
export class FileSystemWritableFileStream extends WritableStream<FileSystemWriteChunkType> {
  constructor(filePath: string, swapPath: string, fd: fs.promises.FileHandle) {
    ...
    let position = 0;
    let currentFd: fs.promises.FileHandle | null = fd;

    super({
      write: async (chunk) => {
        let buffer: Buffer;
        let writePosition: number | undefined = undefined;
        ...
        if (chunk instanceof Blob) {
          ...
          const params = chunk as WriteParams;
          ...
          buffer = Buffer.from(chunk as ArrayBuffer);
